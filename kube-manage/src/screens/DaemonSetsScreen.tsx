...
import { getDaemonSets } from '@/lib/kubernetesClient';
...
export default function DaemonSetsScreen() {
  const navigation = useNavigation<NavProp>();
  const { activeConnection, activeNamespace } = useKubernetes();
  const [search, setSearch] = useState('');

  const { data: daemonSets = [], isLoading } = useQuery<DaemonSetItem[]>({
    queryKey: ['daemonsets', activeConnection?.name, activeNamespace],
    enabled: !!activeConnection,
    staleTime: 15000,
    retry: 1,
    queryFn: async () => {
      const cfg = toParsedConfig(activeConnection!);
      const res = await getDaemonSets(cfg, activeNamespace);
  ...
  const renderItem = ({ item }: { item: DaemonSetItem }) => {
    const allReady = item.ready === item.desired && item.desired > 0;
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => navigation.navigate('DaemonSetDetails', { name: item.name, namespace: item.namespace })}
      >
        <View style={styles.cardMain}>
          <View style={styles.cardLeft}>
            <View style={styles.icon}>
              <Server size={16} color="#FF6B9D" />
            </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsBar}>
        <Text style={styles.statsText}>
          {isLoading ? 'Loading...' : `${filtered.length} daemonset${filtered.length !== 1 ? 's' : ''}`}
        </Text>
        <Text style={styles.statsText}>
          {filtered.filter((d) => d.ready === d.desired && d.desired > 0).length} ready
        </Text>
      </View>
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Server size={40} color="#1E2B42" />
              <Text style={styles.emptyText}>No daemonsets found</Text>
            </View>
          }
        />
      )}
