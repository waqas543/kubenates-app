...
export default function StatefulSetsScreen() {
  ...
  const { data: statefulSets = [], isLoading } = useQuery<StatefulSetItem[]>({
    ...
    queryFn: async () => {
      ...
      const res = await getStatefulSets(cfg, activeNamespace);
  ...
  const renderItem = ({ item }: { item: StatefulSetItem }) => {
    const [ready, total] = item.ready.split('/').map(Number);
    const allReady = !isNaN(ready) && !isNaN(total) && ready === total && total > 0;
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => navigation.navigate('StatefulSetDetails', { name: item.name, namespace: item.namespace })}
      >
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsBar}>
        <Text style={styles.statsText}>
          {isLoading ? 'Loading...' : `${filtered.length} statefulset${filtered.length !== 1 ? 's' : ''}`}
        </Text>
        <Text style={styles.statsText}>
          {filtered.filter((s) => {
            const [r, t] = s.ready.split('/').map(Number);
