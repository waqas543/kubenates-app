...
import { deleteNamespaced, getControllerRevisions, getEvents, getStatefulSet, patchNamespaced } from '@/lib/kubernetesClient';
...
type RouteType = RouteProp<RootStackParamList, 'StatefulSetDetails'>;
...
export default function StatefulSetDetailsScreen() {
  ...
  const { data: sts, isLoading, refetch } = useQuery({
    queryKey: ['statefulset-detail', namespace, name],
    enabled: !!activeConnection,
    queryFn: async () => {
      const res = await getStatefulSet(toParsedConfig(activeConnection!), namespace, name);
  ...
  const handleScale = async (replicas: number) => {
    ...
    try {
      await patchNamespaced(cfg, 'statefulsets', namespace, name, { spec: { replicas } }, 'scale');
      await refetch();
  ...
  const handleRestart = () => {
    Alert.alert('Restart StatefulSet', `Restart "${name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Restart', onPress: async () => {
          ...
          try {
            await patchNamespaced(cfg, 'statefulsets', namespace, name, {
  ...
  const handleDelete = () => {
    Alert.alert('Delete StatefulSet', `Delete "${name}"? This cannot be undone.`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          ...
          try {
            await deleteNamespaced(cfg, 'statefulsets', namespace, name);
            queryClient.invalidateQueries({ queryKey: ['statefulsets'] });
            navigation.goBack();
          } catch (e: any) {
  ...
  const handleRollback = () => {
    Alert.alert('Rollback StatefulSet', `Roll back "${name}" to the previous revision?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Rollback',
        onPress: async () => {
          ...
          try {
            ...
            const ownedCRs = allCRs.filter((cr: any) =>
              (cr.metadata?.ownerReferences ?? []).some(
                (ref: any) => ref.kind === 'StatefulSet' && ref.name === name,
              ),
            );
            ...
            const prevSpec = ownedCRs[1].data?.spec;
            if (!prevSpec) {
              Alert.alert('Error', 'Could not read previous revision spec.');
              return;
            }
            await patchNamespaced(cfg, 'statefulsets', namespace, name, { spec: prevSpec });
            await refetch();
            queryClient.invalidateQueries({ queryKey: ['statefulsets'] });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading statefulset...</Text>
      </View>
    );
  }
  ...
  if (!sts) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.errorText}>StatefulSet not found</Text>
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.header}>
          <View style={styles.headerIcon}><Database size={24} color="#00FF88" /></View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.grid}>
            ...
            <View style={styles.gridCard}><Text style={styles.gridLabel}>Service</Text><Text style={styles.gridValue} numberOfLines={1}>{spec.serviceName ?? '-'}</Text></View>
          </View>
          <View style={styles.detailCard}>
            <DetailRow label="Update Strategy" value={spec.updateStrategy?.type ?? '-'} />
            <DetailRow label="Pod Mgmt Policy" value={spec.podManagementPolicy ?? '-'} />
        ...
        <View style={styles.actions}>
          <TouchableOpacity
            style={[styles.actionBtn, styles.btnGreen, actionLoading === 'scale' && styles.btnDisabled]}
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('Logs', { type: 'statefulset', name, namespace })}
            disabled={!!actionLoading}
          >
...
function createStyles(c: AppColors) {
  return StyleSheet.create({
    ...
    headerIcon: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#00FF8820', alignItems: 'center', justifyContent: 'center' },
    headerInfo: { flex: 1 },
    ...
    modalBox: { backgroundColor: c.bgCard, borderRadius: 16, padding: 24 },
    modalTitle: { fontSize: 20, fontWeight: '700' as const, color: c.text, marginBottom: 6 },
    modalSub: { fontSize: 13, color: c.textSecondary, marginBottom: 20 },
    modalInput: { backgroundColor: c.bgSecondary, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 12, fontSize: 16, color: c.text, borderWidth: 1, borderColor: c.border, marginBottom: 20 },
