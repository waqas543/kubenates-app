...
import { getSecrets } from '@/lib/kubernetesClient';
import { useTheme } from '@/context/ThemeContext';
import type { AppColors } from '@/context/ThemeContext';
import { useNavigation } from '@react-navigation/native';
import type { NativeStackNavigationProp } from '@react-navigation/native-stack';
import { useQuery } from '@tanstack/react-query';
import { ChevronRight, FileKey, Search } from 'lucide-react-native';
...
export default function SecretsScreen() {
  ...
  const { data: secrets = [], isLoading } = useQuery<SecretItem[]>({
    queryKey: ['secrets', activeConnection?.name, activeNamespace],
    enabled: !!activeConnection,
    staleTime: 15000,
    retry: 1,
    queryFn: async () => {
      const cfg = toParsedConfig(activeConnection!);
      const res = await getSecrets(cfg, activeNamespace);
      return (res.data?.items ?? []).map((s: any) => ({
        name: s.metadata?.name ?? 'unknown',
        namespace: s.metadata?.namespace ?? '-',
        type: s.type ?? 'Opaque',
        dataCount: Object.keys(s.data ?? {}).length,
  ...
  const renderItem = ({ item }: { item: SecretItem }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('SecretDetails', { name: item.name, namespace: item.namespace })}
    >
      <View style={styles.cardMain}>
        <View style={styles.cardLeft}>
          <View style={styles.icon}>
            <AnimatedIcon type="swing">
              <FileKey size={16} color={colors.accentRed} />
            </AnimatedIcon>
          </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.statsBar}>
        <Text style={styles.statsText}>
          {isLoading ? 'Loading...' : `${filtered.length} secret${filtered.length !== 1 ? 's' : ''}`}
        </Text>
      </View>
      {isLoading ? (
        <ActivityIndicator size="large" color={colors.accentRed} style={styles.loader} />
      ) : (
        <FlatList
          data={filtered}
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <FileKey size={40} color={colors.border} />
...
function createStyles(c: AppColors) {
  return StyleSheet.create({
    ...
    icon: { width: 36, height: 36, borderRadius: 8, backgroundColor: '#FF575720', alignItems: 'center', justifyContent: 'center' },
    info: { flex: 1 },
